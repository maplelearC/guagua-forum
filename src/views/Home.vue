        ...
        <div class="container">
          ...
          <div class="position-relative" style="padding-top: 20px;">
            ...
            <button v-if="loginUser != null" type="button" @click="dialogVisible = true"
              ...
              style="margin-top: 20px;background-color: #f9d04a; color: #4a73f9" data-target="#publishModal">
              我要发布
            </button>
          </div>
          <!-- 弹出框 -->
          <el-dialog v-model="dialogVisible" title="新帖发布" width="40%" :before-close="handleClose">
            <form>
              <div class="form-group">
                <label for="recipient-name" class="col-form-label">标题：</label>
                <input v-model="this.discussPost.title" type="text" class="form-control" id="recipient-name" />
              </div>
              <div class="form-group">
                <label for="message-text" class="col-form-label">正文：</label>
                <textarea v-model="this.discussPost.content" class="form-control" id="message-text" rows="15"></textarea>
              </div>
            </form>
            <template #footer>
            ...
            <li class="media pb-3 pt-3 mb-3 border-bottom" v-for="(item, index) in listData" :key="index">
              ...
              <div class="media-body">
                <h6 class="mt-0 mb-3">
                  ...
                  }" style="color: #000; text-decoration: none">{{ item.post.title }}&nbsp;&nbsp;</router-link>
                ...
                <div class="text-muted font-size-12">
                  <u class="mr-3">{{ item.user.username }}</u> 发布于
                  <b>{{
                    item.post.createTime.substring(0, 10) +
                    ' ' +
                    item.post.createTime.substring(11, 19)
                  }}</b>
                  <ul class="d-inline float-right">
                    <li class="d-inline ml-2">赞 {{ item.likeCount }}</li>
                    <li class="d-inline ml-2">|</li>
                    <li class="d-inline ml-2">
                      回帖 {{ item.post.commentCount }}
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <!-- 分页 -->
          <el-pagination style="margin-top: 15px; text-align: center" :current-page="page" :page-sizes="[5, 10, 15]"
            :page-size="limit" layout="prev,pager,next,jumper,->,sizes,total" @current-change="handleCurrentChange"
...
export default {
  ...
  methods: {
    ...
    publish(post) {
      ...
      publishAPI(qs.stringify({ title, content }), getToken())
        .then((res) => {
